...
const SearchBloodStockComponent = ({ subheadingText, headingText, classHint, bloodTypeAvailability }) => {
  return (
    <section className={classHint}>
      <h2>{headingText}</h2>
      <p>{subheadingText}</p>

      <div>
        {bloodTypeAvailability.length > 0 ? (
          <ul>
            {bloodTypeAvailability.map((entry, index) => (
              <li key={index}>
                Blood Type: {entry.blood_type}
              </li>
            ))}
          </ul>
        ) : (
          <p>No blood availability data found.</p>
        )}
      </div>
    </section>
  );
